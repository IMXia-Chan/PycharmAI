...
def delete_record(record_id: str, identity: tuple[str, str] = Depends(_identity)):
    """删除一条错误/不规范代码记录。"""
    ok = get_storage().delete_record(record_id, identity[0], identity[1])
    return {"ok": ok}
...
def generate_note(api_key: str = Depends(_api_key), identity: tuple[str, str] = Depends(_identity)):
    ...
    for i, r in enumerate(records, 1):
        lines.append(
            f"【记录 {i}】分类:{r.get('category')}/{r.get('severity')} "
            f"标题:{r.get('title') or r.get('message')}\n"
            f"错误代码:\n```python\n{r.get('code', '').strip()}\n```"
        )
    user = "以下是近期不规范/错误的代码记录:\n\n" + "\n\n".join(lines)
    try:
        content = ai.chat(prompts.NOTE_SYSTEM, user, temperature=0.4, max_tokens=2000, api_key=api_key)
    except Exception as e:  # noqa: BLE001
...
def _library_records(storage, items: list[dict], identity: tuple[str, str]) -> list[dict]:
    """取该文件库里 file 条目(按文件名)与 record 条目(按 ref_id)对应的错误记录。"""
    filenames = {it.get("title") for it in items if it.get("kind") == "file" and it.get("title")}
    record_ids = {it.get("ref_id") for it in items if it.get("kind") == "record" and it.get("ref_id")}
    return [
        r for r in storage.list_records(1000, identity[0], identity[1])
        if (r.get("filename") or "未命名文件") in filenames or r.get("id") in record_ids
    ]


def _library_snippets(storage, items: list[dict], identity: tuple[str, str]) -> list[dict]:
    """取该文件库里 snippet 条目(按 ref_id)对应的代码片段。"""
    snippet_ids = {it.get("ref_id") for it in items if it.get("kind") == "snippet" and it.get("ref_id")}
    return [s for s in storage.list_snippets(1000, identity[0], identity[1]) if s.get("id") in snippet_ids]
...
def generate_library_note(library_id: str, api_key: str = Depends(_api_key), identity: tuple[str, str] = Depends(_identity)):
    """根据该文件库里的错误记录(AI 讲解)与代码片段(原样导出),生成一份库专属笔记。"""
    storage = get_storage()
    ...
    snippets = _library_snippets(storage, items, identity)

    if not records and not snippets:
        return LibraryItem(
            id="", library_id=library_id, kind="note", ref_id="",
            title=f"{lib_name} · 笔记",
            content="这个文件库还没有拖入任何内容。把下方「错误记录」或「代码片段」拖进来,再点「生成笔记」。",
            created_at="",
        )

    parts = []
    # 代码片段:原样导出(本地代码什么样,笔记就什么样),不调 AI
    for s in snippets:
        seg = f"## {s.get('title') or '代码片段'}\n\n```python\n{s.get('code', '')}\n```"
        if s.get("note"):
            seg += f"\n\n{s['note']}"
        parts.append(seg)

    # 错误记录:AI 讲解(错误代码/正确代码/为什么错/举一反三)
    if records:
        if not ai.is_available(api_key):
            raise _http_ai_unavailable()
        lines = []
        ...
        try:
            ai_content = ai.chat(prompts.NOTE_SYSTEM, user, temperature=0.4, max_tokens=2000, api_key=api_key)
        except Exception as e:  # noqa: BLE001
            logger.warning("生成库笔记失败:%s", e)
            ai_content = "AI 笔记生成失败,请检查 DeepSeek API key。"
    ...
    item = storage.add_library_item(
        library_id,
        {"kind": "note", "ref_id": "", "title": f"{lib_name} · 笔记", "content": content},
        identity[0], identity[1],
    )
    return LibraryItem(**item)
