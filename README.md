...
- **Kotlin 插件**（`plugin/`）：IDE 内的一切交互 —— 代码旁内联提示、快捷键、文件库窗口、代码片段窗口、Edge 搜索。
- **Python 后端**（`backend/`）：AI 大脑 —— 调用 DeepSeek 做错误检测 / 中文搜函数 / 函数对比 / AI 问答 / 代码解释 / 自动修复等；并有内置规则兜底，无 API key 也能用基础提示。
- **云端服务**（`server/`，可选）：可选的多用户文件库 / 版本更新提示 / 管理员入口。**默认不使用**，数据存在本机。
...
| 记录错误代码 | — | 把当前选中（或整个文件）记录到本地文件库 |
| 保存 / 打开代码片段 | `Ctrl+Alt+K` / `Ctrl+Alt+W` | 右侧代码片段库 |
| 管理员登录 | — | 用户名 + 密码 + 邮箱验证码（2FA） |
...
4. 在网页端把文件组拖进 5 个候选库，分别「生成笔记」「导出 PDF / Markdown」。

> 「本地文件库」和「网页 AI 笔记候选库」是两回事：本地库是你在 IDE 里攒的原始记录，候选库是临时中转，上传到网页端后才生成笔记。
...
│   └── web/            # 网页工作台（AI 问答、笔记、5 库、导出）
├── server/             # 可选云端服务（多用户文件库 / 版本 / 管理员入口）
